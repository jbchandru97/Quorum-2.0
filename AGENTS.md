...
- The actions tray is the Devin hand-off: header shows "Devin connected" (logo in `src/assets/brands/devin.png`), the `+` enters selection mode (select all / per-row checkboxes) and "Send to Devin" marks rows sent (client-side demo state). The tray footer links to `/quorum/threads`.
