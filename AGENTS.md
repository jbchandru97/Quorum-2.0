...
- Tagged teammates reply automatically after ~1.4s via `simulatedReplyFor` in `src/lib/quorum/demo-script.ts` (scripted lines for scripted beats, persona-plausible otherwise; a reply that tags someone chains once). Agent answers that recommend a human carry a persisted `suggestion` rendered as a one-tap "Ask <name>" bar that sends the tag + question.
