...
- Actions: a message can be captured from its hover `+ Action` control, and a failing usability-review card from its inline `+ Add as action` button. Resolving marks the thread resolved first (system line records by whom), then the agent synthesizes a summary + actions for the scripted targets (`ai-insight-prompt`, `income-card`); summary messages carry a "View action items" link that opens the tray. Actions are removable everywhere (`actions.remove`).
- The actions tray is the Devin hand-off: header reads "action items · N" with a `workspace ↗` redirect beside the close button; below it an action row with a select-all checkbox (left) and an explicit "Handover to Devin" button (right, Devin logo from `src/assets/brands/devin.png`); every action row carries its own checkbox. Handover marks rows `sent` (client-side demo state).
- Demo targets: `ai-insight-prompt` (nudge CTA), `ai-assistant-tab` (sidebar Aql AI item), `income-card` (Total Income widget) — anchored via `data-quorum-target` in the vendored tree and mapped in `fixtures/context/component-map.json`.
